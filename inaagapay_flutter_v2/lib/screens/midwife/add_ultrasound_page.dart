...
class _AddUltrasoundPageState extends State<AddUltrasoundPage> {
  ...
  bool _validateCurrentStep() {
    ...
    if (_step == 0) {
      ...
      if (_pregnancyEdd != null &&
          _date!.isAfter(_pregnancyEdd!.add(const Duration(days: 30)))) {
      ...
      if (location.length < 3 || location.length > 150) {
        _showMessage('Ultrasound location must be 3 to 150 characters.');
        return false;
      }
  ...
  Future<void> _submit() async {
    if (_pregnancyId == null || _date == null) {
      _showMessage('No ongoing pregnancy found.');
      return;
    }
    ...
    try {
      ...
      if (_imageFile != null) {
        ...
        final upload = await _uploadImage(bytes, userId);
      ...
      final inserted = await Supabase.instance.client
          ...
          .insert({
            ...
            'ultrasound_location': _locationCtrl.text.trim(),
            'ultrasound_image': publicUrl,
      ...
      final ultrasoundId = inserted['ultrasound_id'];
  ...
  Future<Map<String, String>> _uploadImage(Uint8List bytes, int? userId) async {
