...
class _MotherChildrenScreenState extends State<MotherChildrenScreen> {
  ...
  Future<void> _fetchChildrenWithMotherId(int motherId) async {
    try {
      ...
      // First, let's check if this mother exists
      final motherCheck = await SupabaseService.client
          .from('mothers')
          .select('mother_id, account_id')
          .eq('mother_id', motherId)
          .maybeSingle();
      
      print('🔍 Mother verification: $motherCheck');

      // Get children with their birth details
      ...
      print('🔍 Children query response: $response');
      print('🔍 Number of children found: ${response.length}');

      // If no children found, let's check what children exist in the database
      if (response.isEmpty) {
        print('🔍 ⚠️ No children found for mother_id: $motherId');
        
        // Check all children to see what mother_ids they have
        ...
        // Specifically check for Juan and Maria
        final juanMaria = await SupabaseService.client
            .from('children')
            .select('child_id, mother_id, first_name, last_name')
            .in_('first_name', ['Juan', 'Maria']);
              ...
              child: Column(
                children: [
                  ...
                  if (_children.isEmpty) ...[
                    const SizedBox(height: 16),
                    Text(
                      'Debug Info: Mother ID: ${_getStoredMotherId()}',
                      style: const TextStyle(
                        fontSize: 12,
                        color: AppColors.textSecondary,
  ...
  // Helper method to show stored mother ID (for debugging)
  String _getStoredMotherId() async {
