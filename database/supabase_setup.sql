...
CREATE TABLE weight_gain_evaluations (
  ...
  status VARCHAR(20) NOT NULL CHECK (
    status IN ('NORMAL', 'LOW', 'HIGH', 'INSUFFICIENT DATA')
  ),
...
CREATE INDEX IF NOT EXISTS idx_notifications_unread ON notifications(account_id, is_read)
WHERE is_read = FALSE;
...
IF v_account_id IS NULL THEN RETURN NEW;
END IF;
...
AFTER
INSERT ON checkup_schedule FOR EACH ROW EXECUTE FUNCTION notify_checkup_scheduled();
...
WHERE v.target_recipients = 'child'
  ...
  AND NOT EXISTS (
    ...
    WHERE ir.child_id = NEW.child_id
      AND ir.vaccine_id = v.vaccine_id
  )
...
AFTER
INSERT ON immunization_record FOR EACH ROW EXECUTE FUNCTION notify_next_vaccine_due();
...
DECLARE v_mother_id BIGINT;
BEGIN IF NEW.next_schedule IS NULL THEN RETURN NEW;
END IF;
...
VALUES (
    v_mother_id,
    NEW.next_schedule,
    'Auto-scheduled from prenatal checkup'
...
AFTER
INSERT ON prenatal_checkups FOR EACH ROW EXECUTE FUNCTION auto_schedule_next_checkup();
-- ============================================================
...
VALUES (
    rec.account_id,
    'Checkup Reminder',
    'Hi ' || rec.first_name || ', your prenatal checkup is on ' || TO_CHAR(rec.scheduled_date, 'Mon DD, YYYY') || '. Please prepare and arrive on time.',
    'checkup_reminder'
  );
END LOOP;
END;
...
DECLARE rec RECORD;
BEGIN FOR rec IN
SELECT DISTINCT ON (c.child_id, v.vaccine_id) c.child_id,
...
ORDER BY c.child_id,
  v.vaccine_id,
  v.recommended_age_months ASC LOOP
INSERT INTO notifications (account_id, title, message, type)
VALUES (
    rec.account_id,
    'Vaccine Due: ' || rec.child_name,
    rec.vaccine_name || ' is due for ' || rec.child_name || ' (recommended at ' || rec.recommended_age_months || ' months, child is now ' || ROUND(rec.age_months::numeric, 1) || ' months). Please visit your BHC.',
    'vaccine_reminder'
  );
END LOOP;
END;
...
CREATE OR REPLACE FUNCTION mark_missed_checkups() RETURNS void AS $$ BEGIN
...
VALUES (
  ...
  (
    ...
    6.0,
    'child',
    'Vitamin A Supplementation - 1st dose'
  ),
  (
    'Vitamin A',
    2,
    12.0,
    'child',
    'Vitamin A Supplementation - 2nd dose'
  ),
  (
    'Measles',
    2,
...
SELECT cron.schedule(
    'daily-checkup-reminders',
    '0 0 * * *',
    'SELECT send_upcoming_checkup_reminders()'
  );
SELECT cron.schedule(
    'daily-vaccine-reminders',
    '0 0 * * *',
    'SELECT send_vaccine_due_reminders()'
  );
SELECT cron.schedule(
    'daily-mark-missed-checkups',
    '5 0 * * *',
    'SELECT mark_missed_checkups()'
  );
SELECT cron.schedule(
    'day-before-checkup-sms',
    '0 22 * * *',
    'SELECT send_day_before_checkup_sms()'
  );